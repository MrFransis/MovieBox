...
from MovieBoxServer.server import start_server
from MovieBoxServer.classifier import exportClassifier
from classifier import LemmaTokenizer

class App(cmd.Cmd):
    intro = 'MovieBox server launched. \n \nType help or ? to list commands.\n'
    prompt = '>'
    neo4j_driver = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "root"))

    def do_retriveMovie(self, arg):
        'Download movies'
    ...
    def do_initDB(self, arg):
        ...
        films_path = './data/dataset.csv'

        # Initialization of utils
        session = self.neo4j_driver.session()
        users_df = pd.read_json(users_path, lines=True)
        films_df = pd.read_csv(films_path, encoding="ISO-8859-1", on_bad_lines='skip', low_memory=False)
